...
const { ensureLoggedIn, ensureAdmin, ensureAdminOrLoggedInUser } = require("../middleware/auth");
...
router.patch("/:username", ensureAdminOrLoggedInUser, async function (req, res, next) {
  try {
...
router.delete("/:username", ensureAdminOrLoggedInUser, async function (req, res, next) {
  try {
